...
If we have a posterior, $p(\theta | y_1, \ldots y_n)$ we can sample from and we want some summary of the posterior... e.g. $p(\theta > a)$, quantiles of the posterior, or the posterior of some transform $f(\theta)$, all of these can be mapped to some sort of integral e.g. $\int_{\infty}^a p(\theta) d\theta$ and Monte Carlo approximation can be used. In each of these cases, the procedure is the same.
