...
a. Find the prior distirbution for $\gamma$ induced by our prior on $\theta$. Is the prior informative about $\gamma$? Verify $p(\gamma)$ using Monte Carlo sampling and then plotting the empirical density of the samples along with the closed-form solution.
...
b. The scientists run the machine $n$ times. Let $y_i$ be one if the machine fails on the $i$th run, and zero otherwise. Write out the posterior distribution of $\theta$ given $y_1, \ldots, y_n$ and simplify as much as possible. 

c. For the case that $n = 4$, make a plot of the posterior of $\theta$ for the five cases $\sum y_i \in \{ 0, 1, 2, 3, 4\}$.

d. The posterior is a mixture (weighted average) of two distributions that you know. Identify these two distributions.
