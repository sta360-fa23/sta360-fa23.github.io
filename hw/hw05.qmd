...
a. For the case that $\theta \sim \text{gamma}(a, b)$, show that the posterior mean of $\theta$ given $Y_,1 \ldots Y_n$ can be written as $\hat{\theta}_w = w \bar{y} + (1-w)\mu$ for values $w$ and $\mu$ that depend on $n$, $a$ and $b$. 
