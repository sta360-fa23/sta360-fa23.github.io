...
- simplify $\boldsymbol{b}^T A \bt + \bt ^T A \boldsymbol{b}$
- if $\bt$ is a $p \times 1$ vector, what is the dimension of the result?
...
$$
where $S_\theta = \sum_{i=1}^n (\by_i - \bt)(\by_i - \by)^T$ is the residual sum of squares matrix for the vectors $\by_1, \ldots \by_n$ if the population mean is $\bt$.
...
Exercise/show offline: why does this work? Hint: what is $cov(\boldsymbol{Z}^T \boldsymbol{Z})$?

We can automate the above with
