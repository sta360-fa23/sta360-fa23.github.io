...
If we consider the posterior precision $\lambda_n^2 \equiv \frac{1}{\tau_n^2}$, we can re-arrange the terms above to illuminate how posterior information = prior information + data information;.

$$
{\lambda_n^2} = \lambda_0^2 + n \lambda^2
$$
- ${\lambda_n^2}$: posterior precision
- $\lambda_0^2$: prior precision
- $\lambda^2$: sampling precision
...
But how do we choose $p(\sigma^2)$ to be conjugate? We can proceed in two ways: one is noting that the integral is really a convolution of normals and is therefore a normal density (for some intution, see [this 3Blue1Brown video on convolutions](https://www.youtube.com/watch?v=IaSGqQa5O-M). The second way is perhaps more easily illustrated.
...
and since we have found $\theta | \sigma^2 \sim N(\mu_0, \sigma^2/\kappa_0)$, for conjugacy with fixed $\sigma$, we need to choose $p(\sigma^2)$ to preserve the form of $p(\theta | \sigma^2)$.
