...
2. update $\sigma^2$:
  ...
  - sample $1/\sigma^2 \sim \text{gamma}([\nu_0 + n]/2, [\nu_0 \sigma_0^2 + SSR(\beta)]/2)$
...
We might have figured out this is what we were going to see already based on the the fact that the expressions for $E[\beta | \mathbf{y}, X]$ and $Var[\beta | \mathbf{y}, X]$ look just like OLS when the prior information is diffuse.
...
$$
\hat{\beta}_{ridge} = \underset{\beta}{\mathrm{argmin}} \underbrace{(\mathbf{y} - X\beta)^T (\mathbf{y} - X \beta)}_{\text{SSR}(\beta)} + \underbrace{\lambda \beta^T \beta}_{L_2 ~\text{penalty}}
$$
...
$$
Exercise: show that $\hat{\beta}_{ridge} = E[\beta | \mathbf{y}, X] = (X^TX)^{-1} + \lambda I)^{-1} X^T \mathbf{y}$.
