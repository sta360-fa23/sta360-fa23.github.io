...
## fixed values ## 
mu = c(-3, 0, 3) # conditional means
s2 = rep(1 / 3, 3) # conditional variance
d = c(1, 2, 3) # sample space of delta
N = 1000 # chain length
w = c(.45, .1, .4) # delta probabilities

delta = sample(d, size = N, prob = w, replace = TRUE)
theta = rnorm(N, mean = mu[delta], sd = sqrt(s2[delta]))
...
- **effective sample size**: intuitively this is the effective number of exact samples
"contained" in the Markov chain (see [Betancourt 2018](https://arxiv.org/pdf/1701.02434.pdf)). In practice we use `coda::effectiveSize()` function to compute. Example:
...
$$
In words, it's the number of independent Monte Carlo samples necessary to give the same precision as the MCMC samples.
